...
class DeviceManager
{
    ...
    getDeviceId(device)
    {
        ...
        if (device)
        {
            if (typeof device === 'string')
            {
                ...
                if (this.deviceTopics && normalizedDevice && this.deviceTopics.has(normalizedDevice)) { return this.deviceTopics.get(normalizedDevice); }
            }
            else if (typeof device === 'object')
            {
                ...
                if (device.name)
                {
                    ...
                    if (this.deviceTopics && normalizedDeviceName && this.deviceTopics.has(normalizedDeviceName)) { return this.deviceTopics.get(normalizedDeviceName); }
                }
            }
        }
    ...
    async getDeviceById(deviceId)
    {
        if (deviceId)
        {
			// Get the device from this.devices that was loaded during registration
	...
	{
		let wasConnected = this.api.devices.isConnected();
		if (!wasConnected)
		{
			try
			{
    ...
    async registerDevice(device)
    {
        if (typeof device === 'object')
        {
            ...
            if (deviceTopic)
            {
                ...
                const topicDeviceId = this.deviceTopics.get(deviceTopic);
                if (topicDeviceId && topicDeviceId !== device.id)
                {
                    this.app.updateLog(`[registerDevice] Duplicate normalized topic '${deviceTopic}' for '${deviceName}' (${device.id}). Keeping original mapping to ${topicDeviceId}.`, 0);
                }
                else
                {
    ...
    async _updateDevice(deviceOrId)
    {
        const deviceId = (typeof deviceOrId === 'object' && deviceOrId !== null) ? deviceOrId.id : (deviceOrId && deviceOrId.id ? deviceOrId.id : deviceOrId);
        let updatedDevice = (typeof deviceOrId === 'object' && deviceOrId !== null) ? deviceOrId : null;

        if (deviceId)
        {
            // Prefer a fresh device snapshot from the API to keep capabilitiesObj up to date.
            try
            {
            ...
            {
                this.app.updateLog(`_updateDevice: Error fetching updated device ${deviceId}: ${error.message}`, 0);
            }

            if (updatedDevice)
            {
                if (this.zones && updatedDevice.zone)
                {
                    updatedDevice.zone = this.zones[updatedDevice.zone] || updatedDevice.zone;
                }

                this.devices = this.devices || {};
                this.devices[deviceId] = updatedDevice;
            }
        }

        await this.onUpdate.emit(updatedDevice || deviceOrId);
    }
    ...
    {
        let deviceObj = await this.getDevice(deviceId);
		if (!deviceObj)
		{
			return undefined;
		}
		...
		if (deviceObj.capabilities)
		{
			...
			{
				this.app.updateLog(`Error getting capabilities for ${deviceObj.name}`);
				this.app.updateLog(error);
			}
		}
