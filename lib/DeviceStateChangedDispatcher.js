...
class DeviceStateChangeDispatcher
{
    ...
    async registerDeviceCapability(device, capabilityId)
    {
        ...
        {
            this.app.updateLog(`Error registering device capability: missing device (${device}) or capabilityId (${capabilityId})`, 0);
            return false;
        }
            ...
            for (let device of Object.values(devices))
            {
                if (device.checkStateChange)
                {
                    try
                    {
                        ...
                        await device.checkStateChange(deviceId, capability, value);
                    }
                    catch (error)
                    {
